...
#Lang which has word → index (word2index) and index → word (index2word) dictionaries,
...
class Lang:
    ...
    def loadVocab(self):
        with open(self.vocab_path, encoding="utf-8") as f:
            rawData = f.readlines()
            vocab = list(map(lambda word: word[:-1], rawData)) #remove \n
        vocab_norm = [normalizeString(word) for word in vocab] #normalize
...
def get_dataloader(batch_size):
    ...
    TRAIN_PATH = ['data/trainset/train_en.txt',
                  'data/trainset/train_vi.txt']
    VOCAB_PATH = ['data/vocabularies/vocab_en.txt',
                  'data/vocabularies/vocab_vi.txt']
    TEST_PATH = ['./dataset/testset/tst2013_en.txt',
                 './dataset/testset/tst2013_vi.txt']
...
def train_epoch(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion):
...
def train(encoder, decoder, n_iters, print_every=100, plot_every=100, learning_rate=0.001):
    start = time.time()
    plot_losses = []
    ...
    # Print "Saved the model" only once at the end of training
    encoder_path = f'./model/encoder_{model_name}.pt'
    decoder_path = f'./model/decoder_{model_name}.pt'
    print(f'Training complete.\nModel saved: {encoder_path}, {decoder_path}.')
...
def BlueScore(encoder, decoder, pairs):
...
if __name__ == '__main__':
    ...
    model_name = 'Vitnamese'
    TRAIN_PATH = ['data/trainset/train.en.txt',
                  'data/trainset/train.vi.txt']
    VOCAB_PATH = ['data/vocabularies/vocab.en.txt',
                  'data/vocabularies/vocab.vi.txt']
    TEST_PATH = ['data/testset/tst2013.en.txt',
                 'data/testset/tst2013.vi.txt']
    ...
    if opt.mode == 'train':
        train(encoder1, attn_decoder1, 4000, print_every=100)
        ...
        _, _, test_pair = prepareData(TEST_PATH[0], TEST_PATH[1], VOCAB_PATH[0], VOCAB_PATH[1], False,
                                      max_length=MAX_LENGTH)
        print("BLEU score:" + str(BlueScore(encoder1, attn_decoder1, test_pair)))
